...
input_features = ["股票代码","交易日期","开盘价","最高价"]
different_axis = ("交易日期","股票代码", "收益率",)
################################################################################################
# 构建训练集
total_train_df = pd.DataFrame()
begin_year = pd.Timestamp('2018-01-01')
...
_open_train = _open.loc[begin_year:stop_year].dropna(axis = 1)
_high_train = _high.loc[begin_year:stop_year].dropna(axis = 1)
_ret_train = _ret.loc[begin_year:stop_year].dropna(axis = 1)
...
eval_df = pd.DataFrame([
                         _open.loc[eval_begin_year:eval_stop_year, _open_train.columns].fillna(0).stack().rename('开盘价'),
                         _high.loc[eval_begin_year:eval_stop_year, _high_train.columns].fillna(0).stack().rename('最高价'),
                         _ret.loc[eval_begin_year:eval_stop_year, _ret_train.columns].fillna(0).stack().rename('收益率'),
                         ]).T
